...
const UserTools = () => {
  ...
  const fetchContacts = async () => {
      try {
          ...
          if(Array.isArray(data)) {
              // අලුත්ම අය උඩට එන විදිහට Sort කරනවා
  ...
  const handleCreateCampaign = async () => {
    ...
    if(!window.confirm(`Schedule "${campaignName}" for ${selectedContacts.length} people using this Template? (This may charge Meta fees)`)) return;
                        ...
                        <div className={`${cardColor} p-6 rounded-3xl border`}>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex bg-black/20 p-1 rounded-xl">
                                    <button onClick={() => setUseTemplate(false)} className={`flex-1 py-2 text-sm font-bold rounded-lg transition ${!useTemplate ? 'bg-emerald-600 text-white' : 'text-slate-400'}`}>Free Broadcast (24h)</button>
                                    <button onClick={() => setUseTemplate(true)} className={`flex-1 py-2 text-sm font-bold rounded-lg transition ${useTemplate ? 'bg-blue-600 text-white' : 'text-slate-400'}`}>Meta Template (Paid)</button>
                                </div>
                                ...
                                {!useTemplate && (
                                    <div className="bg-emerald-500/10 border border-emerald-500/20 p-4 rounded-xl mb-2">
                                        <p className="text-emerald-400 text-sm flex items-start gap-2">
                                            <Zap className="mt-0.5" size={16} />
                                            <span><strong>Free Broadcast Mode:</strong> This will instantly send a standard WhatsApp message (no template fees) to ALL users who messaged you in the last 24 hours.</span>
                                        </p>
                                    </div>
                                )}
                                ...
                                <button onClick={handleCreateCampaign} disabled={sending || uploading} className={`w-full py-4 text-white font-bold rounded-xl shadow-lg flex items-center justify-center gap-2 transition disabled:opacity-50 ${useTemplate ? 'bg-gradient-to-r from-blue-600 to-indigo-600 hover:opacity-90' : 'bg-emerald-600 hover:bg-emerald-500'}`}>
                                    ...
                                    {sending ? 'Processing...' : (useTemplate ? 'Schedule Paid Template' : 'Send Free Broadcast Now')}
                                </button>
                            </div>
                        </div>
                    ...
                    <div className={`${cardColor} p-6 rounded-3xl border flex flex-col h-[800px]`}>
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2">
                            {filteredContacts.map(contact => {
                                ...
                                return (
                                    <div key={contact._id} onClick={() => toggleSelect(contact._id)} className={`flex items-center justify-between p-3 rounded-xl cursor-pointer border transition ${isSelected ? 'bg-blue-600/10 border-blue-500/50' : `border-transparent hover:${isDark ? 'bg-white/5' : 'bg-gray-50'}`}`}>
